...
void BellmanFord(struct Graph* graph, int source){
    ...
    int StoreDistance[V],i,j;
    for (i = 0; i < V; i++)
        StoreDistance[i] = INT_MAX;
    StoreDistance[source] = 0;
    for (i = 1; i <= V-1; i++){
        for (j = 0; j < E; j++){
            int u = graph->edge[j].source, v = graph->edge[j].destination, weight = graph->edge[j].weight;
            if (StoreDistance[u] + weight < StoreDistance[v])
                StoreDistance[v] = StoreDistance[u] + weight;
        }
    }
    for (i = 0; i < E; i++){
        int u = graph->edge[i].source, v = graph->edge[i].destination, weight = graph->edge[i].weight;
        if (StoreDistance[u] + weight < StoreDistance[v])
